...
- (id)init
{
    ...
    if (self = [super initWithFrame:CGRectMake(0, 0, 320, _h)])
    {
        // background settings
...
- (void)sizeToFitOrientation:(UIInterfaceOrientation)orientation
{
    ...
    else
    {
        ...
        [panelView setFrame:self.frame];
        [containerView setFrame:self.frame];
        [webView setFrame:self.frame];
        [indicatorView setCenter:CGPointMake(160, self.frame.size.height*0.5)];
...
- (void)bounceInAnimationStopped
{
    ...
    [closeButton setFrame:CGRectMake(self.bounds.size.width-45, self.bounds.size.height-45, 35, 35)];
    //[closeButton setImageEdgeInsets:UIEdgeInsetsMake(3, 0, 0, 0)];
    [closeButton setImage:[UIImage imageNamed:@"ShareKit.bundle/closeweibo.png"] forState:UIControlStateNormal];
